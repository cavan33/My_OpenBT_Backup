...
preds <- x # For in-sample prediction
library(Ropenbt)
...
# summarize_fits(fits)
# save_fits(fits, 'RScripts/Results/Walmart/fits_result.txt')

# Save objects:
fpath = 'RScripts/Results/Walmart/'
...
# For non-in-sample (not working)
# c(ys, yhats) <- set_up_plot(fitp, x, y, points = length(x), var = c(0, 1, 2, 3), day_range = 30)

# In-sample plot:
y_df <- data.frame("y" = y, "yhat" = fitp$mmean)
lims_df <- data.frame("x" = c(0.1, 3.9), "y" = c(0.1, 3.9))
pred_plot(y_df, 'In-Sample BART y vs. y-hat, Full Settings',
          'RScripts/Plots/Walmart/y-yhat-in-sample1',
          ms = 0.1, lims = lims_df)